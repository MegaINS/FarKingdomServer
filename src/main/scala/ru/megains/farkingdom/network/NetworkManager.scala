...
import ru.megains.farkingdom.network.packet.login.SPacketDisconnect
import ru.megains.farkingdom.network.packet.{ConnectionState, Packet, PacketRead}



class NetworkManager(server:FKServer) extends SimpleChannelInboundHandler[PacketRead[INetHandler]] with Logger[NetworkManager] {
